...
/**
 * Automatically generates a JDialog with the class fields
 * @author Rosário Pereira Fernandes
 * @version 1.0.0
 * @since 1.0.0
 *
 */
public class JForm extends JDialog{
	...
	/**
	 * Class Constructor.
	 * @param object - your DAO
	 * @param onConfirm - actionListener for the confirmation button.
	 */
	public JForm(Class object, ForeignKey [] keys, ActionListener onConfirm)
	{
									...
									if(fields[i].isAnnotationPresent(Secure.class))
									{
										...
										if(fields[i].isAnnotationPresent(Date.class))
										{
													...
													annotation.ForeignKey.class))
											{
												...
												addComboBox(label, options, isEnabled);
											}
											else
												addSpinner(label, isEnabled);
					}
					else //Handling booleans
						if(type.equalsIgnoreCase("boolean"))
						{
							addCheckBox(label, isEnabled);
						}
						else //Handling reals
	...
	private void addTextField(String label, boolean isEnabled)
	{
	...
	private void addSpinner(String label, boolean isEnabled)
	{
		JLabel jlabel = new JLabel(label);
	...
	private void addCheckBox(String label, boolean isEnabled)
	{
		JLabel jlabel= new JLabel(label);
		getContentPane().add(jlabel,"2, "+currentRow+", right, default");
	...
	private void addComboBox(String label, String [] options, boolean isEnabled)
	{
		JLabel jlabel= new JLabel(label);
		getContentPane().add(jlabel,"2, "+currentRow+", right, default");
	...
	private void addFormattedTextField(String label, String format, boolean isEnabled)
	{
		JLabel jlabel= new JLabel(label);
		jlabel.setEnabled(isEnabled);
		getContentPane().add(jlabel,"2, "+currentRow+", right, default");
	...
	private void addPasswordField(String label, char echoChar, boolean isEnabled)
	{
		JLabel jlabel= new JLabel (label);
		getContentPane().add(jlabel,"2, "+currentRow+", right, default");
	...
	private void addDateChooser(String label, String format, boolean isEnabled)
	{
	...
	private void addTextArea(String label, boolean isEnabled)
	{
		addLabel(label, isEnabled);
	...
	public void clear()
	{
		...
		{
			String component=components[i].getClass().getSimpleName();
			if(component.equalsIgnoreCase("JTextField") || component.equalsIgnoreCase("JFormattedTextField") || 
					component.equalsIgnoreCase("JPasswordField"))
			{
	 ...
	 * @return data - the data inserted on the form
	 */
